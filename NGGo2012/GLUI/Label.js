...
{ /** @lends GLUI.Label.prototype */
    ...
    setTextSize: function (size)
    {
        ...
        if(this._internalGLObject)
        {
            ...
            this._fontScale = (size / this._FIXED_FONT_SIZE) + 0.0000001;
            this._internalGLObject.setFontSize(this._FIXED_FONT_SIZE);

            var bsize = this._internalGLObject.getSize();
            var validSize = (bsize.getWidth() > 0) && (bsize.getHeight() > 0);
            if (validSize) {
                var width = this._sizeText.getWidth();
                var height = this._sizeText.getHeight();

                if ((width == 0) || (height == 0)) {
                    this._sizeText = bsize.clone();
                    width = this._sizeText.getWidth();
                    height = this._sizeText.getHeight();
               }

                this._internalGLObject.setSize(width / this._fontScale, height / this._fontScale);
            }
            this._internalGLObject.setScale(new Core.Vector(this._fontScale, this._fontScale));
        }
    ...
    _renderView: function ()
    {
        ...
        var width = this._frame[2] - right - left;
        var height = this._frame[3] - bottom - top;
        this._sizeText.setWidth(width);
        this._sizeText.setHeight(height);
