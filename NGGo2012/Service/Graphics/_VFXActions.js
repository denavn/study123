...
exports.VFXTaskNode = Class.subclass( /** @lends Service.Graphics.VFXTaskNode.prototype */
{
     ...
     * @param {number} trackId to have the tracking id of current TaskNode.
     * @param {Function} call back when called on finish.
     */
    initialize: function (node, trackId)
    {
        this.node = node;
        this.func = null;
        this.args = [];
     ...
     */
    and: function (func, args, trackId)
    {
        if(func === "blink")
        {
        ...
        this.func = func;
        this.args = args;
        var nextTask = new exports.VFXTaskNode(this.node, trackId);
        this.next = nextTask;
        return nextTask;
    },
    /**
...
exports.VFX = MessageListener.singleton( /** @lends Service.Graphics.VFX.prototype */
{
    ...
    enchant: function (node, trackId)
    {
        ...
        var newTask = new exports.VFXTaskNode(node, trackId);
     ...
     *
     * @param {GL2.Node} node Target node object.
     * @param {bool} stopAllActions to decide whether to stop all tasks or current task.
     * @param {number} trackId to have the tracking id of remaining tasks.
     */
    stop: function (node, stopAllActions, trackId)
    {
        if(node._dnVFXManagedId)
        {
            if(stopAllActions)
            ...
            {
                var key;
                for(key in this.tasks[node._dnVFXManagedId])
                {
                    if(this.tasks[node._dnVFXManagedId].hasOwnProperty(key))
                    {
                        if(this.tasks[node._dnVFXManagedId][key].trackId === trackId)
                        {
                            delete this.tasks[node._dnVFXManagedId][key];
                        }
                    }
                }
            }
            if(node._previousAlpha)
            {
                node.setAlpha(node._previousAlpha);
            }
        }
    },
    /**
