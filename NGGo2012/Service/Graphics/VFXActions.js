...
var Sequence = Core.Class.subclass( /** @lends Service.Graphics.Sequence.prototype*/
{
     ...
     * 
     * @param {number} trackId to have the tracking id of current spawn/sequence.
     * @see Service.Graphics.VFXActions
     * */
    initialize: function (actions, trackId)
    {
    ...
    /**
     * Stops current sequence on the provided node.
     *
    ...
    stop: function (node)
    {
        ...
        {
            VFX.stop(node, false, this._trackId);
        }
        else
        {
    ...
    _handleRepeat: function (context)
    {
        ...
        {
            var trackId = context._rootTrackId || context._trackId;
            if(context._parent && context._hasMaxDuration)
            {
                context._parent._index = 0;
                VFX.enchant(context._node, trackId).and(context._parent._handleRepeat, [context._parent], trackId);
            }
            else if(context._parent && context._parent.classname !== "Spawn")
            {
                VFX.enchant(context._node, trackId).and(context._parent._execute, [context._parent], trackId);
            }
            else
            {
                context._progress = 0;
    ...
    _execute: function (context)
    {
        ...
        if(context.actions[context._index])
        {
            ...
            var resetSS = function (context)
                {
                    context.actions[context._index]._rootTrackId = context._rootTrackId || context._trackId;
            ...
            {
            case "Action":
                var vfxTask = VFX.enchant(context._node, trackId).and(context.actions[context._index].func, context.actions[context._index].args, trackId);
                context._index++;
                while(context.actions[context._index] && context.actions[context._index].classname === "Action")
                {
                    vfxTask = vfxTask.and(context.actions[context._index].func, context.actions[context._index].args, trackId);
                    context._index++;
                }
                if(context.actions.length === context._index)
                {
                    vfxTask.and(context._handleRepeat, [context], trackId);
                }
                else
                {
                    vfxTask.and(context._execute, [context], trackId);
                }
                break;
            case "Sequence":
            case "Spawn":
                resetSS(context);
                VFX.enchant(context._node, trackId).and(context.actions[context._index]._handleRepeat, [context.actions[context._index]], trackId);
                context._index++;
                break;
            }
        }
        else
        {
            if(context.actions.length === context._index)
            {
                VFX.enchant(context._node, trackId).and(context._handleRepeat, [context], trackId);
            }
        }
    }
});
...
var Spawn = Core.Class.subclass( /** @lends Service.Graphics.Spawn.prototype*/
{
    ...
    /**
     * Stops current spawn of passed node.
     *
     * @param {GL2.Node} node Target node object.
     */
    stop: function (node)
    ...
    _execute: function (context)
    {
        ...
        if(context.actions[context._index])
        {
            ...
            case "Action":
                ...
                {
                    VFX.enchant(context._node, trackId).and(context.actions[context._index].func, context.actions[context._index].args, trackId).and(context._handleRepeat, [context], trackId);
                }
                else
                {
                    VFX.enchant(context._node, trackId).and(context.actions[context._index].func, context.actions[context._index].args, trackId);
                }
                if(context.actions.length !== context._index + 1)
                {
                    VFX.enchant(context._node, trackId).and(context._execute, [context], trackId);
                }
                context._index++;
                break;
            case "Sequence":
...
var VFXActions = Core.Class.singleton( /** @lends Service.Graphics.VFXActions.prototype*/
{
    ...
    {
        this._trackId++;
        return new exports.Sequence(actions, this._trackId);
    },
    /**
    ...
    {
        this._trackId++;
        return new exports.Spawn(actions, this._trackId);
    },
    /**
     * Pauses all actions. 
     */
