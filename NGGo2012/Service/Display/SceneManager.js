...
var SceneManagerImpl = ServerSync.subclass(
/** @lends Service.Display.SceneManager.prototype */
{
     ...
     */
    /**
     * Sets the screen name and resets the complete root node.
     * @param {String} name The screen name to be set.
     * @example <code>
     * ...
     * setScreenName("myScreen")
     * ...
     * </code>
     * @function
     */
    setScreenName: function(name)
    {
        ...
        var oldRoot = this._nodes.GL2_ROOT;
        var newRoot = this._screenManager.settings[name].getRootNode();
        var key;
        for (key in this._nodes)
        {
            if (this._nodes.hasOwnProperty(key))
            {
                ...
                if ((node !== GL2.Root) && (node.getParent() === oldRoot))
                {
                    newRoot.addChild(node);
                }
            }
        }
        this._nodes.GL2_ROOT = newRoot; 
    },
    /**
    ...
    {
        if(!this._nodes[key] && node !== GL2.Root)
        {
    ...
    {
        if(node !== GL2.Root && this._nodes[key] && this._nodes[key] !== GL2.Root)
        {
            this._replaceNode(this._nodes[key], node, key);
        }
    },
    ...
    insertNodeForKey : function(key, node, nodeKey)
    {
        if(node !== GL2.Root && this._nodes[key] && this._nodes[key] !== GL2.Root)
        {
            this._insertNode(key, this._nodes[key], nodeKey, node);
        }
    },
    ...
    _replaceNode : function(oldNode, newNode, key)
    {
        ...
        else
        {
            ...
            var oldChildren = oldNode.getChildren();
            var idx = 0;
            for(idx = 0; idx < oldChildren.length; ++ idx)
            {
                oldNode.removeChild(oldChildren[idx]);
                newNode.addChild(oldChildren[idx]);
            }
    ...
    __onLoadData : function(data, parent)
    {
        ...
        if (data.screenName)
        {
            this.setScreenName(data.screenName);
        }
        if (parent === undefined)
        {
            parent = this._nodes.GL2_ROOT;
        }
        var error;
        var prop;
        for(prop in data)
...
exports.SceneManagerImpl = SceneManagerImpl;
exports.SceneManager = SceneManagerImpl.singleton({});
