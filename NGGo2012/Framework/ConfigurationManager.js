...
var KeyListenerForBack = Core.MessageListener.singleton(
{
    ...
    {
        if(keyEvent.code === KeyEmitter.Keycode.back && this.backCommand)
        {
            this.backCommand();
            return true;
        }
...
{
    /**
     * Error code of <code>ConfigurationManager</code> class.
     * @namespace ERROR
     */
    ERROR: {
        INVALID_PATH: 2,
    ...
    begin: function (callback)
    {
        ...
        fs.readFile(this.mConfigFile, false, function (err, data)
        {
            if(err)
            {
                ...
                console.log("<NGGO> Error: Could not read the file " + self.mConfigFile);
                error = new NGGOError(self.ERROR.INVALID_PATH, "ConfigurationManager: Error occured when reading, " + self.mConfigFile);
                callback(error);
            }
            else
            {
                ...
                {
                    console.log("<NGGO> Error: Could not parse the JSON file " + self.mConfigFile);
                    error = new NGGOError(self.ERROR.PARSE_ERROR, "ConfigurationManager: JSON Parse error in, " + self.mConfigFile + "  " + ex);
                    callback(error);
                }
                self.mConfig = config;
    ...
    _loadMultiManifestManager: function (callback)
    {
        ...
        {
            MultiManifestManager.start(this.mConfig.MultiManifestManager, function (manifestIdx, inProgress, stillToGo, total, name)
            {
