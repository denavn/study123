...
var GLUISpinner = require('../../GLUI/Spinner').Spinner;
var GLUIWindow = require('../../GLUI/Window').Window;
exports.SplashScreenManager = ServerSync.singleton( /** @lends Service.Display.SplashScreenManager.prototype */
{
    ...
    start: function (callBackFunc, node)
    {
        ...
        if (this._splashData)
        {
            ...
            //add and play sound
            if (typeof (this._splashData.sound) === "string")
            {
                this._addSound("bgMusic", this._splashData.sound);
                this._bgMusic = true;
            }
            else if (this._splashData.sound !== undefined && this._splashData.sound !== null)
            {
                throw new Error('Sound path should be a string. Got ' + typeof (this._splashData.sound) + '.');
            }
            this.next();
        }
        else
    ...
    abort: function ()
    {
        ...
        this._shouldWait = true;
        this._spinner = null;
        this._splashSpinner = null;
        this._initialBackground = null;
        this._isRunning = false;
        var callBackFunc = this._callBackFunc;
        this._callBackFunc = null;
        ...
        if (this._splashData)
        {
            ...
            else
            {
                ...
                if (splashes.sound)
                {
                    ...
                    {
                        throw new Error('Sound path should be a string. Got ' + typeof (splashes.sound) + '.');
                    }
                }
                //checks touch target and splashes.text
                if (this._touchTarget && splashes.text)
                {
                    this._insertTapText();
                }
                ...
                if (splashes.image instanceof Object)
                {
                    if (!this._image)
                    {
                        ...
                        this._image.setPosition(this._screenSize[0] / 2, this._screenSize[1] / 2);
                    ...
                    if (splashes.spinner)
                    {
                        if (splashes.spinner.image)
                        {
                            this._splashSpinner = new GLUISpinner();
                            this._splashSpinner.setImage(splashes.spinner.image);
                            GLUIWindow.document.addChild(this._splashSpinner);
                        }
                        else
                        {
                            this._splashSpinner = new Spinner();
                            Window.document.addChild(this._splashSpinner);
                        }
                        if (splashes.spinner.frame)
                        {
                            this._splashSpinner.setFrame(splashes.spinner.frame);
                        }
                        else
                        {
                            ...
                            if (this._screen)
                            {
                                screenSize = this._screen.convert(screenSize);
                            }
                            var size = Math.min(screenSize[0], screenSize[1]);
                            this._splashSpinner.setFrame([(screenSize[0] - size / 5) * 0.5, (screenSize[1] - size / 5) * 0.5, size / 5, size / 5]);
                        }
                    }
                }
                else
    ...
    /** @private */
    _insertTapText: function ()
    {
        var splashes = this._splashData.splashes[this._count];
        if (splashes.text)
        {
            this._tapText = new Text();
            this._tapText.setDepth(10);
            this._tapText.setAnchor(splashes.text.anchor || [0.5, 0.5]);
            this._tapText.setColor(splashes.text.color || [0, 0, 0]);
            this._tapText.setFontFamily(splashes.text.font || 'MarkerFelt');
            this._tapText.setPosition(splashes.text.position || [this._screenSize[0] * 0.5, this._screenSize[1] * 0.5]);
            this._tapText.setFontSize(splashes.text.fontSize || 20);
            this._tapText.setText(splashes.text.text || "Tap to View Next");
            this._splashView.addChild(this._tapText);
        }
    },
    /** @private */
    ...
    /** @private */
    _clearSplashSpinner: function ()
    {
        if (this._splashSpinner)
        {
            this._splashSpinner.destroy();
            this._splashSpinner = undefined;
        }
    },
    /** @private */
    ...
    __onLoadData: function (splashData)
    {
        ...
        if (this._splashData.screenName)
        {
            ...
            // Sync with ScreenManager load.
            if (ScreenManager.hasGLViewLoaded)
            {
                this.start();
            }
            else
            {
                ScreenManager._interruptCallback(this.start.bind(this, ScreenManager._callback));
            }
            return true;
        }
    },
