...
/** @lends Service.Data.ServerSync */
{
    ...
    loadConfigFromFile: function (path, param, callback)
    {
        ...
        else
        {
            if(callback)
            {
                callback(new NGGOError(this.ERROR.INVALID_PATH, self.classname + ": `path` should be URI object or URI string."));
            }
        }
    },
    /**
    ...
    loadConfigFromAsset: function (assetKey, param, callback)
    {
        ...
        {
            return new NGGOError(this.error.ASSET_NOT_FOUND, this.classname + ": '" + assetKey + "' is not found.");
        }
    },
    /**
    ...
    _parseJSON: function (data, callback)
    {
        ...
        case "string":
            ...
            {
                return new NGGOError(this.ERROR.PARSE_ERROR, this.classname + ": Input data is invalid as JSON.");
            }
            try
            {
            ...
            {
                console.log(ex2.stack);
                returnvalue = new NGGOError(this.ERROR.CALLBACK_ERROR, this.classname + ": Error occured in callback -> " + ex2.toString());
            }
            break;
        case "object":
            ...
            break;
        default:
            returnvalue = new NGGOError(this.ERROR.INVALID_TYPE, this.classname + ": Input data should be Object or JSON string.");
            break;
        }
        return returnvalue;
    },
    ...
    _startWatchLocalFile: function (file, callback)
    {
        ...
        req.addObserver(
        {
            ...
            onFailure: function (res)
            {
                ...
                {
                    callback(new NGGOError(self.ERROR.FILE_NOT_FOUND, self.classname + ": [404] File not found"));
                }
                else
                {
                    callback(new NGGOError(self.ERROR.NETWORK_ERROR, self.classname + ": [" + req.status + "] Network Error"));
                }
            }
        });
                        ...
                        self.loadConfigFromFile(url, function (err)
                        {
                            ...
                            else
                            {
                                ...
                                case self.ERROR.PARSE_ERROR:
                                    err = new Error("JSON from server could not be parse." + " Please check your data and try again");
                                    console.log(err);
    ...
    __readLocalFile: function (filepath, callback)
    {
        ...
        FileSystem.readFile(filepath, false, function (err, data)
        {
            if(err)
            {
                callback(new NGGOError(self.ERROR.FILE_READ_ERROR, self.classname + ": File '" + filepath + "' not found."));
            }
            else
            {
                callback(null, data);
